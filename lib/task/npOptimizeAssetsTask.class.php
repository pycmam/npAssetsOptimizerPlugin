...
class npOptimizeAssetsTask extends sfBaseTask
{
  ...
  public function configure()
  {
    ...
    $this->addOptions(array(
      ...
      new sfCommandOption('conf', null, sfCommandOption::PARAMETER_REQUIRED, sprintf('The configuration of assets to optimize (%s)', implode(', ', self::$types)), $default = 'all'),
    ));
  ...
  {
      $configurations = sfConfig::get('app_np_assets_optimizer_plugin_configuration', array());

      if ($options['conf'] == 'all') {
          foreach ($configurations as $name => $conf) {
              $this->logSection('configuration', $name);
              $this->executeConfiguration($conf, $arguments, $options);
          }
      } else {
          $this->executeConfiguration($configurations[$options['conf']], $arguments, $options);
      }
  }

  protected function executeConfiguration($serviceConfiguration, $arguments = array(), $options = array())
  {
